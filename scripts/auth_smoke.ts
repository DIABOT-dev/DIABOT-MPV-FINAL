...
async function mustGET(path: string, expect2xx: boolean) {
  ...
  if (expect2xx) {
    assert(res.ok, "GET " + path + " expected 2xx, got " + res.status);
  }
  return res;
}
...
async function softPOST(path: string, body: any, expect2xx: boolean) {
  ...
  if (expect2xx) {
    assert(res.ok, "POST " + path + " expected 2xx, got " + res.status);
  }
  return res;
}
...
async function main() {
  ...
  assert(health.ok || health.status === 200, "Health should be reachable");
  ...
  if (haveAuthRoutes) {
    ...
    assert(res.ok, "Login expected 2xx, got " + res.status);
  } else {
  ...
  if (haveAuthRoutes) {
    assert(res.ok, "log water expected 2xx, got " + res.status);
  } else {
  ...
  if (haveAuthRoutes) {
    ...
    assert(res.ok, "forgot expected 2xx, got " + res.status);
    ...
    if (res.ok) {
      ...
      assert(res2.ok, "re-login expected 2xx, got " + res2.status);
    }
  } else {
  ...
  if (haveAuthRoutes) {
    ...
    assert(res.ok || res.status === 204, "logout should be 2xx/204");
  } else {
